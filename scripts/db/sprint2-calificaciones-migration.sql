...
CREATE TABLE IF NOT EXISTS evaluacion (
    ...
    CONSTRAINT uq_evaluacion_asignacion_periodo_nombre
        UNIQUE (id_asignacion_docente, periodo, nombre)
);
...
CREATE INDEX IF NOT EXISTS idx_evaluacion_institucion_asignacion
    ON evaluacion (id_institucion, id_asignacion_docente, periodo);
...
CREATE INDEX IF NOT EXISTS idx_calificacion_inscripcion
    ON calificacion (id_inscripcion);
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger WHERE tgname = 'trg_evaluacion_actualizado_en'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger WHERE tgname = 'trg_calificacion_actualizado_en'
    ) THEN
